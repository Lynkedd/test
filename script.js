...
if (quoteForm) {
  quoteForm.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      // Formspree file uploads require a paid plan. Keep the photo picker UI,
      // but omit files so quote-request emails still send on the free plan.
      // Change this to true after enabling file uploads in Formspree.
      const formspreeFileUploadsEnabled = false;
      const hadSelectedPhotos = chosenFiles.length > 0;
      if (!formspreeFileUploadsEnabled) {
        formData.delete('attachment');
        formData.append(
          'photoNote',
          hadSelectedPhotos
            ? `${chosenFiles.length} project photo(s) were selected but were not uploaded. Please contact the customer to request them.`
      ...
      if (formStatus) {
        formStatus.textContent = hadSelectedPhotos && !formspreeFileUploadsEnabled
          ? 'Thank you! Your estimate request was sent. Your selected photos were not uploaded, so TWilliams may ask you to email them separately.'
          : 'Thank you! Your estimate request was sent successfully. TWilliams will contact you soon.';
